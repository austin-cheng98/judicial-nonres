...
if res is not None:
    ...
    lexc = res[(res["model"] == "lexical-rule") & (res["split"] == "COURT")]
    if len(lexc):
        mac("LexCourt", f"{lexc['macro_f1'].max():.3f}")
...
if bs_ is not None and len(bs_):
    ...
    hs = bs_[(bs_["stratum"] == "H1") & (bs_["context"] == "SENT")]
    if len(hs):
        mac("TfidfHOne", f"{hs['tfidf_acc'].iloc[0]:.3f}")
    ps = bs_[(bs_["stratum"] == "PLAIN") & (bs_["context"] == "SENT")]
    if len(ps):
        mac("TfidfPlain", f"{ps['tfidf_acc'].iloc[0]:.3f}")
        mac("LexPlain", f"{ps['lexical_acc'].iloc[0]:.3f}")
...
jd = load("18_judge_diag.json", "json")
if jd:
    mac("JudgeTrigUnres", pct(jd["trigger_rate_unresolved"], 1))
    mac("JudgeTrigDec", pct(jd["trigger_rate_decided"], 1))
    mac("JudgeTrigEnrich", f"{jd['trigger_enrichment']:.0f}")
    mac("JudgeAccLowMatch", f"{jd['acc_match_low']:.3f}")
    mac("JudgeAccHighMatch", f"{jd['acc_match_high']:.3f}")

im = load("21_issue_match.json", "json")
if im:
    mac("NCharMatched", num(im["n"]))
