...
python3 scripts/benchmark/08_worksheet.py ingest

echo "$(date +%T) === step 9: sparse models and context test ==="
python3 scripts/models/09_models.py

echo "$(date +%T) === step 10a: issue-specific citation chains ==="
python3 scripts/analysis/10a_chains.py

echo "$(date +%T) === step 9c: zero-shot language model ==="
python3 scripts/models/09c_llm.py || echo "!! 09c failed, continuing"

echo "$(date +%T) === step 9b: fine-tuned encoders ==="
python3 scripts/models/09b_encoder.py || echo "!! 09b failed, continuing"
